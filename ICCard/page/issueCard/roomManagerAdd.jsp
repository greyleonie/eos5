...
<body onLoad="Initial();" onUnload="cardcontrol.Quit()">
	<form method="post" name="roomManager" action="ICCard.prIssueCard.roomManagerAddDO.do">
        ...
        if (layerNO < 0) {
        %> <input type="radio" name="Card/CardTypeID" value="1" onclick="ShowLayerNO()" />楼层卡 <input type="radio"
									name="Card/CardTypeID" value="2" checked="true" onclick="HideLayerNO()" />楼房总管卡 <%} else {
        %> <input
									type="radio" name="Card/CardTypeID" value="1" checked="true" onclick="ShowLayerNO()" />楼层卡 <input type="radio"
									name="Card/CardTypeID" value="2" onclick="HideLayerNO()" />楼房总管卡 <%}
    %></td>
						</tr>
