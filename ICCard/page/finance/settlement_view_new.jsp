...
%>
	<head>
		...
		<!--调用新建的打印js代码-->
		<script language="JavaScript" src="/internet/scripts/SelectNewPrint.js"></script>
		...
		<script language="JavaScript">
			...
			function setPrintArray(){
				...
				//alert("rownum=" + rownum + ",colnum=" + colnum);
				for(var row = 0; rownum > row; row++){
					...
					//alert("_iscashbusiness=" + _iscashbusiness + "_businessmoney=" + _businessmoney);
				...
				//初始化SelectNewPrint
				printer = new SelectNewPrint("printer", "_printSetting", printPaper, -10, 45, data, title);
				printer.layout();
			}
		...
		<table id="listdetail" WIDTH="96%" border="0" cellspacing="1" cellpadding="0" bgcolor="#A2C4DC" align="center">
			...
			<%
			 if (base.util.TalentContext.getEntity(pageContext,null,"list[@type='bizLogCols']")!=null) 
			 {
			  	...
			  	String operator = "";
			  	String length = base.util.TalentContext.getValue(pageContext,null,"QueryDayBizLogCols/listLength");
			  	int listLength = Integer.parseInt(length, 10);
			  	int count = 0;			  	
			  	String departId = base.util.TalentContext.getValue(pageContext,null,"QueryDayBizLogCols/ChargeDepartmentID/criteria/value");
			%>
				<logic:iterate id="businesslogscoll" property="list[@type='bizLogCols']">
					<%
						count++;
						date = base.util.TalentContext.getValue(pageContext,"businesslogscoll","QueryDayBizLogCols/HappeningDate");
						if(oldDate != "" && !oldDate.equals(date) && departId.equals("1"))
						{
					%>
						<tr align="center" class='td_title'>	
							...
							<td nowrap align='center' valign='middle'>
									<%
									out.println("<font color='#B6BDC3'>");
									out.println(String.valueOf(potSum));
									out.println("</font>");
									//potSum = 0;
									 %>
									<!--<input type="hidden" name="BusinessMoney" value="0"><!-- fix it-->
									<input type="hidden" name="BusinessMoney" <%out.println("value='" + String.valueOf(potSum) + "'");%>>
									<%potSum=0; %>
									<input type="hidden" name="IsCashBusiness" value="2"><!-- fix it-->
							</td>
							<td nowrap align='center' valign='middle'>
								<div>
					...
					<%
						}//end if(oldDate != "" && !oldDate.equals(date))
						else
						{
							// sum POT money
							businessTypeID = base.util.TalentContext.getValue(pageContext,"businesslogscoll","QueryDayBizLogCols/BusinessTypeID");
							operator = base.util.TalentContext.getValue(pageContext,"businesslogscoll","QueryDayBizLogCols/Operator");
							if (businessTypeID != null && businessTypeID.equals("3") && operator != null)
							{
								String money = base.util.TalentContext.getValue(pageContext,"businesslogscoll","QueryDayBizLogCols/BusinessMoney");
								float temp = (money != null) ? Float.parseFloat(money) : 0;
								potSum += temp;
							}
						}
						oldDate = date;
					...
					%>
						<td nowrap align='center' valign='middle'>
							...
							<div>
								...
								<%
								String happenDate = base.util.TalentContext.getValue(pageContext,"businesslogscoll","QueryDayBizLogCols/HappeningDate");
								out.println(happenDate);
								 %>
							</div>
						</td>
						<td nowrap align='center' valign='middle'>
						...
						<td nowrap align='center' valign='middle'>
							...
							businessTypeID = base.util.TalentContext.getValue(pageContext,"businesslogscoll","QueryDayBizLogCols/BusinessTypeID");
							operator = base.util.TalentContext.getValue(pageContext,"businesslogscoll","QueryDayBizLogCols/Operator");
							if (businessTypeID != null && businessTypeID.equals("3"))
								out.println("<font color='#B6BDC3'>");
							else
								out.println("<font color='#000000'>");
							String bmoney = base.util.TalentContext.getValue(pageContext,"businesslogscoll","QueryDayBizLogCols/BusinessMoney");
							if(bmoney != null)
								out.println(bmoney);
							out.println("</font>");
							 %>
					...
					<%
					if(count == listLength  && departId.equals("1"))
					{
					%>
						<tr align="center" class='td_title'>	
							<td nowrap align='center' valign='middle'>
			...
			<% 
			}//end if (base.util.TalentContext.getEntity(pageContext,null,"list[@type='bizLogCols']")!=null)
			%>
			</table>
			<%
				if(j>0) {
...
function tablexml(tableObj) {
	...
	for(var i = 0; i < allrows.length; i++) {
		...
		for(var j = 0; j < somerow.cells.length; j++) {
			...
			if (i>0 && j==0) 
				somecellvalue = somecell.children.item(1).innerText;
			else if (i>0 &&j==2) 
				somecellvalue = somecell.children.item(2).value;
			else if (i>0 && (j!=0 || j!=2)) 
				somecellvalue = somecell.children.item(0).innerText;
			else 
				somecellvalue = somecell.innerHTML;
