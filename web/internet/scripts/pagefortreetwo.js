...
function showCalendar(p) {
	...
	var v=binding.value && !binding.title ? binding.realValue.replace(/\-/g,"/"):"";
	var mask=binding.dtype.substr(2).split(",")[0];
	var height=(/[hms]+/).exec(mask) ? 248:228;
	var r=window.showModalDialog("Calendar.html",[v,mask,binding.toMax=="true"],"help:0;resizable:0;status:0;center:1;dialogHeight:"+height+"px;dialogWidth:186px");
	if (r) {
		binding.value=r;
		top.disposeField(binding);
		binding.value=binding.showValue;
	}
	binding.focus();
}
