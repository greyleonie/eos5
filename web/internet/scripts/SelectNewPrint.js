function SelectNewPrint(objectName, displayId, printPaper, fontSize, pageLines, data, titles)
{
	// 记录对象状态
...
function SelectPrint_splitPrint(printWindow)
{
	...
	for (var row = 0; row < this.totalLines; row++) {
		...
		if (row % this.pageLines == 0) {
			...
			//表格的字体调为13个像素
			printWindow.document.write("<thead class='" + this.getTableTitleStyle() + "'><tr style='font-size: 13px'>" + printTitle(this.titles) + "</tr></thead>");
			// 开启表体
...
{
	//表格字体也调为13个像素
	var returnString="<tr style='font-size: 13px'>";	
	for (var index = 0; index < titles.length; index++) {
		if (titles[index][1]){			
