...
创建人：ajun
时间		修改人		说明
2005-06-01  zhouming    增加了支持第四层菜单支持
*/
...
{
    //初始化四级菜单 add by zhouming
	var curDiv=eval("four");
	...
	{   
		...
		for(j=1;j<MenuTwo[3]+1;j++)
		{
			...
			for(k=1;k<MenuThreeArray[3]+1;k++)
			{
                ...
                if (SubMenu[3]>0) htmlString+="onclick=OverThreeMenu(this,'Menu"+i+"_"+j+"_"+k+"');";
                 htmlString+=">";
...
function OverThreeMenu(threeUpMenu,menu){
	var upMenu = eval(threeUpMenu).parentNode.parentNode.parentNode.parentNode;
	for (var j=0;j<upMenu.parentNode.childNodes.length ;j++ ){
		upMenu.parentNode.childNodes[j].style.backgroundColor=bgBarSelect;
	}
	upMenu.style.backgroundColor=bgBarFour;
	var curDiv=eval("four");
	curDiv.style.visibility="visible";
	var MenuFourArray =eval(menu);
	var divFourSpan = eval("four_span");
	var htmlString="";
				htmlString+="<div id='DIV"+menu+"'><table  style='POSITION: absolute;' width='100%' cellspacing='0' cellpadding='0'>";
				htmlString+="<tr>";
				htmlString+="<td><table height='24' border='0' cellpadding='0' cellspacing='0' bgcolor='"+bgBarFour+"'>";
				htmlString+="<tbody><tr>";
				for(l=1;l<MenuFourArray[3]+1;l++)
				{
					SubMenu=eval(menu+"_"+l);
					htmlString+="<td>";
					htmlString+="<table width='"+three_offset+"' border=0 cellpadding=0 cellspacing=0>"; 
					htmlString+="<tr> "; 
					htmlString+="<td align=center onMouseOver=changeBgColor(this,'over'); onMouseOut=changeBgColor(this,'out'); onMouseUp=changeTitle(this.innerText,'"+SubMenu[1]+"','"+SubMenu[2]+"');>";
					...
					htmlString+="</td>"; 
					htmlString+="</tr>";  
					htmlString+="</table></div></td> ";  
				}
				htmlString+="</tr></tbody></table></td></tr></table>";
       divFourSpan.innerHTML=htmlString;
}

//显示4级菜单项目
function showForthMenu()
{
	document.write('<div id=four style="width:100%;VISIBILITY:hidden;background:'+bgBarFour+';" >')
	document.write('<table width="100%" height="24" border="0" cellpadding="0" cellspacing="0">')
	document.write('<tr><td>');
	document.write('<div id="four_div" style="width:'+three_offset*(getSystemMatric()>2?7:5)+';height:24;background:'+bgBarFour+';overflow:hidden;">');
	document.write('<div id="four_span" style="position:relative;left:0;">');	
	document.write('<nobr>');
	htmlString="";
	for(i=1;i<FirstMenuCount+1;i++)			
	{   
		...
		for(j=1;j<MenuTwo[3]+1;j++)
		{
			MenuThree=eval("Menu"+i+"_"+j);
	       for (k=1;k<MenuThree[3]+1 ;k++ )
			{
				MenuFour = "Menu"+i+"_"+j+"_"+k;
				MenuFourArray=eval(MenuFour);
                if(MenuFourArray[3]<1) continue;
				htmlString+="<div id='DIV"+MenuFour+"'><table  style='POSITION: absolute;' width='100%' cellspacing='0' cellpadding='0'>";
				htmlString+="<tr>";
				htmlString+="<td><table height='24' border='0' cellpadding='0' cellspacing='0' bgcolor='"+bgBarFour+"'>";
				htmlString+="<tbody><tr>";
				for(l=1;l<MenuFourArray[3]+1;l++)
				{
					SubMenu=eval("Menu"+i+"_"+j+"_"+k+"_"+l);
					htmlString+="<td>";
					htmlString+="<table width='"+three_offset+"' border=0 cellpadding=0 cellspacing=0>"; 
					htmlString+="<tr> "; 
					htmlString+="<td align=center onMouseOver=changeBgColor(this,'over'); onMouseOut=changeBgColor(this,'out'); onMouseUp=changeTitle(this.innerText,'"+SubMenu[1]+"','"+SubMenu[2]+"');>";
