...
function RePos(){
	if(NoOffFirstLineMenus<=0) return;
	...
	//modify by wangcq
	var count=NoOffFirstLineMenus/MenuLineCount;
	if((NoOffFirstLineMenus % MenuLineCount)!=0){
		count=count+1;
	}
	...
	var i=1;
	while(Cntnr!=null){
		PosMenu(Cntnr,StartTop+(count-i)*22,StartLeft);
		Cntnr=Cntnr.Pre;
		i++;
	}
	//end modify
...
function Go(){
	...
	*/
	...
	for(var i=1;i<=count;i++){
		//if(FrstCreat){
			FrstCntnr=CreateMenuStructure('Menu',NoOffFirstLineMenus,MenuLineCount*(i-1)+1);
			FrstCreat=AcrssFrms?0:1;
			FrstCntnr.Level=1;	
			FrstCntnr.Pre=PreCntnr;
			PreCntnr = FrstCntnr;
		//}
		//else CreateMenuStructureAgain('Menu',NoOffFirstLineMenus);
		if(TargetLoc)ClcTrgt();
		...
		PosMenu(FrstCntnr,StartTop+(i-1)*22,StartLeft);		
		//PosMenuEx(FrstCntnr);
		IniFlg=1;
		Initiate();		
	}
	document.LastCntnr=FrstCntnr;
	Creatd=1;
	...
	AfterBuild();
	if(WriteToFile==1)
		wirte_file(document.body.innerHTML);
...
function Initiate(){
	if(IniFlg){	
		...
		for(var i=0;i<iframes.length;i++){
			var selects = document.frames(iframes[i].name).document.getElementsByTagName("select");
			for(var j=0;j<selects.length;j++){
				selects[j].style.visibility="visible";
			}
		}
		*/
		ShowSelect(document,true);
...
function GoTo(){
	SaveSubPath();
	if(this.LinkTxt=='#') return;
	if(this.LinkTxt){
		if(this.LinkTxt.indexOf("?")==-1)
			this.LinkTxt = this.LinkTxt+"?hciCurMenuPath="+GetCookie("hciCurMenuPath")+"&hciTopMenuPath="+GetCookie("hciTopMenuPath");
		else
			this.LinkTxt = this.LinkTxt+"&hciCurMenuPath="+GetCookie("hciCurMenuPath")+"&hciTopMenuPath="+GetCookie("hciTopMenuPath");
		status='';
		...
		//this.LinkTxt.indexOf('javascript:')!=-1?eval(this.LinkTxt):DcLoc.location.href=this.LinkTxt;		
		parent.frames[MenuTarget].location.href=this.LinkTxt;
		Window.refresh();
		MenuClicked=true;
		showWaiting();
	}
...
function OpenMenu(){	
	...
	*/
	...
	if(this.Container.Level==1){
		...
		window.first = this.value;
		SubPathNum = 1;
	}else{
		SetSubPath(this.Level,this.value);
...
function CntnrSetUp(Wdth,Hght,NoOff){
	...
	if(Nav4){	
		...
		//this.style.backgroundColor=ContainerBgColor
...
function CreateMenuStructure(MName,NumberOf,Start){
	...
	if(Start==0){
		...
		/*
		for(i=0;i<Start+MenuLineCount-End;i++){
			if(DomYes){
				Mbr=Location.document.createElement("div");
				Mbr.style.position='absolute';
				Mbr.style.visibility='inherit';
				Mbr.style.width=Wdth;
				Mbr.style.height=Hght;
				MmbrCntnr.appendChild(Mbr);
			}
		}
		*/
...
function SaveSubPath(){
	switch(SubPathNum)
	{
		case 2: document.total	= window.first + "¡ú" + window.second;break;
		case 3: document.total	= window.first + "¡ú" + window.second + "¡ú" + window.third;break;
		case 4: document.total	= window.first + "¡ú" + window.second + "¡ú" + window.third + "¡ú" + window.four;break;
		case 5: document.total	= window.first + "¡ú" + window.second + "¡ú" + window.third + "¡ú" + window.four + "¡ú" + window.fifth;break;
		case 6: document.total	= window.first + "¡ú" + window.second + "¡ú" + window.third + "¡ú" + window.four + "¡ú" + window.fifth + "¡ú" + window.sixth;break;
		case 7: document.total	= window.first + "¡ú" + window.second + "¡ú" + window.third + "¡ú" + window.four + "¡ú" + window.fifth + "¡ú" + window.sixth + "¡ú" + window.seventh;break;
