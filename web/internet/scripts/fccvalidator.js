...
// 验证是否为时间HH:mm:ss形式  add by csd
function isStandardTime( strValue )
{
	
	// 日期格式必须是 HH:mm:ss  不为空
	if( isEmpty( strValue ) ) return false;

	if( !checkExp( /^[0-2]?\d:[0-5]?\d:[0-5]?\d$/g, strValue ) ) return false;
...
function isPhone( strValue )
{
	...
	return checkExp( /(^\(\d{3,5}\)\d{6,8}(-\d{2,8})?$)|(^\d+-\d+$)|(^(130|131|132|133|134|135|136|137|138|139|150|151|152|153|154|155|156|157|158|159)\d{8}$)/g, strValue );
}
...
function ValidateEl( obj, strType)
{
	...
	switch( strType.toLowerCase() )
	{
		...
		case "timeonly":  //时间 HH:mm
			result=	isTimeOnly(strValue);
			break;
			
		case "standardtime" : //时间 HH:mm:ss
			result=	isStandardTime(strValue);
			break;
		...
		case "maxlength" :
			...
			result=checkMaxlength(strValue,maxlength);
			break;
	}
...
function selectDateNoTime() {
	var str=window.showModalDialog("/talent/talent/js/getdateNoTime.html",null,"font-size:10px;dialogWidth:180px;dialogHeight:180px;scrollbars=no;status=no");
...
function selectDate() {
	var str=window.showModalDialog("/talent/talent/js/getdate.html",null,	"font-size:10px;dialogWidth:180px;dialogHeight:180px;scrollbars=no;status=no");
	if (str == null || str == "" || str == "undefined")
		return "";
	else
		return str;
...
/*
*---------------------------------------------------------------------------------------------------
*文本框获得焦点时运行该方法:onfocus="textValidate()"
*可以对:url,email,int,float,mobile,alpha,money1,decimal
*
*-------------------------------------------------------------------------------------------------
*/
