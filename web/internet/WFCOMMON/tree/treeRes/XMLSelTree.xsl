...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	<xsl:variable name="LeafImgSrc">/internet/BNOM/tree/mgrTreeRes/images/NodeImg0.gif</xsl:variable>
	<xsl:variable name="BranchImgSrc">/internet/BNOM/tree/mgrTreeRes/images/NodeImg1.gif</xsl:variable>
	<xsl:template match="/">
		<xsl:apply-templates/>
	</xsl:template>
				...
				<tr>
					<td nowrap="true">
						...
						<span class="clsLabel" type="label" onclick="MouseClick(this)" onmousedown="NodeMouseDown(this);" onmouseover="NodeMouseOver(this);" onmouseout="NodeMouseOut(this);">
							...
							<xsl:attribute name="title"><xsl:choose><xsl:when test="@Caption"><xsl:value-of select="@Caption"/></xsl:when><xsl:otherwise><xsl:value-of select="$title"/></xsl:otherwise></xsl:choose></xsl:attribute>
