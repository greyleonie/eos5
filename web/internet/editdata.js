...
//  Composition.document.write(contont);
  //"<head><style type=\"text/css\">body {font-size: 10.8pt}</style><meta http-equiv=Content-Type content=\"text/html; charset=gb2312\"></head><BODY bgcolor=\"#FFFFFF\" MONOSPACE></body>");
...
{ 
if (!validateMode()) return;
 Composition.document.execCommand("InsertHorizontalRule","dddd","分页符");
...
{
  if (!	validateMode())	return;
  var arr = showModalDialog("table.html", "", "dialogWidth:20.5em; dialogHeight:16.5em; status:0");
  ...
  colo=ss[3]
  var string;
  string="<table width='100%' border="+ss[2]+">";
  for(i=1;i<=row;i++){
  string=string+"<tr>";
  for(j=1;j<=col;j++){
  string=string+"<td  bgcolor="+colo+"></td>";
  }
  string=string+"</tr>";
  }
  string=string+"</table>";
...
function help()
{
    var helpmess;
    helpmess="---------------填写帮助---------------\r\n\r\n"+
         "1.已注册的用户请先输入正确的昵称和密码\r\n\r\n"+
         "2.新用户请去注册\r\n\r\n"+
         "3.标题不能超过128个字符,若超过则自动截断.\r\n\r\n"+
         "4.出处不能超过128个字符,若超过则自动截断.";
    alert(helpmess);

}


function Insermytable()
{
  if (!	validateMode())	return;
  Composition.focus();
  var range = Composition.document.selection.createRange();
  var arr = showModalDialog("table.ASP",window, "dialogWidth:300pt;dialogHeight:236pt;help:0;status:0");
...
function Insermenu(id)
{
  ...
  Composition.focus();
  var range = Composition.document.selection.createRange();
  var ran = Composition.document.selection.createRange("").text;
  var arr = showModalDialog("insmenu.htm?id="+id, "", "dialogWidth:450pt;dialogHeight:186pt;help:0;status:0");
