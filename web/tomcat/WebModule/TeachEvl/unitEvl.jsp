...
<%
  ...
  String title="",explain="",remarks="",deadline="",unitName="",unitdpmt="",unittime="",className="",openState="";
  ...
  //读取单元信息
  sql = "select a.*,IsNull(b.orgName,'') orgName from TeachingPlanUnit a left join EOSORG_T_Organization b on b.orgID=a.Department where a.UnitID=" + unitID;
  ...
  //读取评估表信息
  sql = "select * from Evaluation where evaluationid in (select top 1 evaluationid from AssignMark where classid="+classID+" and evaluationid in (select evaluationid from Evaluation where period='单元' and classtype='1'))";
  ...
  if ("".equals(title)) {
  sql = "select top 1 * from Evaluation where ClassType='1' and Period='单元' and (Class like '%"+classID+"%' or Class like '%"+classID+",%' or Class like '%,"+classID+",%' or Class like '%,"+classID+"%')";
  ...
  if ("".equals(title)) {
  sql = "select top 1 * from Evaluation where ClassType='1' and Period='单元' order by EvaluationID desc";
  Evaluation = stmt.executeQuery(sql);
  while (Evaluation!=null && Evaluation.next()) {
     evlID = Evaluation.getInt("EvaluationID");
     isadvice = Evaluation.getInt("IsNeedAdvice");
		  ...
		  <td><table id="allCourse" width="100%" border="0" cellpadding="1" cellspacing="1" bgcolor="#000000">
            ...
            <tr>
              ...
              <td width="30%" align="center" bgcolor="#FFFFFF">
                <textarea name="item<%=itemId%>_suggest" cols="70" rows="5" ><%=suggest%></textarea>
              </td>
              <% } %>
            </tr>
            <% } %>
...
<% } %>
<input type="button" id="back" value="返回" onclick="window.parent.location.href='zlpg.jsp?openState=<%=openState%>'">
</div>
</form>
</body>
