...
<body>
	<form method="post" name="myform" action="Research.pr.prResearchCityProcessDo.do" enctype="multipart/form-data">
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
			<tr>
				<td height="5">
					...
					<html:hidden property="WCMMetaTableCityPaperApp/WCMMetaTableCityPaperAppID" />
					<html:hidden property="WCMMetaTableCityPaperApp/participant" />
					<html:hidden property="WCMMetaTableCityPaperApp/passParticipant" />
		...
		%>
		  ...
		  <input name="select" type="button" class="button_02" value="选 择" onClick="selectParticipant('<bean:write property="WFWorkItem/processInstID"/>','<bean:write property="WFWorkItem/processDefID"/>','10000','MR_APPLY/nextParticipantName','MR_APPLY/nextParticipantId')">-->
									<%if (display) {
		%>
									<html:text property="WCMMetaTableCityPaperApp/nextParticipantName"
										attributesText='style="width:80%;border:1 #efefef soild;" readonly="true"' />

									<input name="select" type="button" class="button_02" value="选 择"
										onClick="selectParticipant('<bean:write property="WFWorkItem/processInstID"/>','<bean:write property="WFWorkItem/processDefID"/>','10000','WCMMetaTableCityPaperApp/nextParticipantName','WCMMetaTableCityPaperApp/nextParticipantId')">
									<%} else {
		%>
									<html:text property="WCMMetaTableCityPaperApp/nextParticipantName"
		...
		%></td>
						...
						<html:hidden property="WCMMetaTableCityPaperApp/nextParticipantId" />
					</table>
				</td>
			</tr>
			</td>
...
function returnNode(){
	...
	if(isSure==true){
		...
		try{
		document.getElementById('WCMMetaTableCityPaperApp/nextParticipantName').value='<bean:write property="ExtendData/PassedNode/UserID"/>';
		}catch(e){
		
		}
		document.getElementById('WCMMetaTableCityPaperApp/nextParticipantId').value='<bean:write property="ExtendData/PassedNode/UserID"/>';
	...
	var frm = document.forms[0];
		...
		if(validater(frm)) {
			if(getRadioValue("Flow/nextNode")!="EndActivity"){
				if(frm.elements["WCMMetaTableCityPaperApp/nextParticipantId"].value == ""){
					alert("请选择下环节人员");
					return;
				}
			}
