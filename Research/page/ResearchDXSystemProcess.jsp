...
<body>
	<form method="post" name="myform" action="Research.pr.prResearchDXSystemProcessDo.do" enctype="multipart/form-data">
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
			<tr>
				<td height="5">
					...
					<html:hidden property="WCMMetaTableschoolPaperApp/WCMMetaTableschoolPaperAppID" />
					<html:hidden property="WCMMetaTableschoolPaperApp/participant" />
					<html:hidden property="WCMMetaTableschoolPaperApp/passParticipant" />
		...
		%>
		  ...
		  <input name="select" type="button" class="button_02" value="选 择" onClick="selectParticipant('<bean:write property="WFWorkItem/processInstID"/>','<bean:write property="WFWorkItem/processDefID"/>','10000','MR_APPLY/nextParticipantName','MR_APPLY/nextParticipantId')">-->
									<%if (display) {
		%>
									<html:text property="WCMMetaTableschoolPaperApp/nextParticipantName"
										attributesText='style="width:80%;border:1 #efefef soild;" readonly="true"' />

									<input name="select" type="button" class="button_02" value="选 择"
										onClick="selectParticipant('<bean:write property="WFWorkItem/processInstID"/>','<bean:write property="WFWorkItem/processDefID"/>','10000','WCMMetaTableschoolPaperApp/nextParticipantName','WCMMetaTableschoolPaperApp/nextParticipantId')">
									<%} else {
		%>
									<html:text property="WCMMetaTableschoolPaperApp/nextParticipantName"
		...
		%></td>
						...
						<html:hidden property="WCMMetaTableschoolPaperApp/nextParticipantId" />
					</table>
				</td>
			</tr>
			</td>
...
function returnNode(){
	...
	if(isSure==true){
		...
		try{
		document.getElementById('WCMMetaTableschoolPaperApp/nextParticipantName').value='<bean:write property="ExtendData/PassedNode/UserID"/>';
		}catch(e){
		
		}
		document.getElementById('WCMMetaTableschoolPaperApp/nextParticipantId').value='<bean:write property="ExtendData/PassedNode/UserID"/>';
	...
	var frm = document.forms[0];
		...
		if(validater(frm)) {
			if(getRadioValue("Flow/nextNode")!="EndActivity"){
				if(frm.elements["WCMMetaTableschoolPaperApp/nextParticipantId"].value == ""){
					alert("请选择下环节人员");
					return;
				}
			}
