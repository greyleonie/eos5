...
<body>
	<form method="post" name="myform" action="Research.pr.prResearchCountryProcessDo.do" enctype="multipart/form-data">
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
			<tr>
				<td height="5">
					...
					<html:hidden property="WCMMetaTablecountryPaperApp/WCMMetaTablecountryPaperAppID" />
					<html:hidden property="WCMMetaTablecountryPaperApp/participant" />
					<html:hidden property="WCMMetaTablecountryPaperApp/passParticipant" />
		...
		%>
		  ...
		  <input name="select" type="button" class="button_02" value="选 择" onClick="selectParticipant('<bean:write property="WFWorkItem/processInstID"/>','<bean:write property="WFWorkItem/processDefID"/>','10000','MR_APPLY/nextParticipantName','MR_APPLY/nextParticipantId')">-->
									<%if (display) {
		%>
									<html:text property="WCMMetaTablecountryPaperApp/nextParticipantName"
										attributesText='style="width:80%;border:1 #efefef soild;" readonly="true"' />

									<input name="select" type="button" class="button_02" value="选 择"
										onClick="selectParticipant('<bean:write property="WFWorkItem/processInstID"/>','<bean:write property="WFWorkItem/processDefID"/>','10000','WCMMetaTablecountryPaperApp/nextParticipantName','WCMMetaTablecountryPaperApp/nextParticipantId')">
									<%} else {
		%>
									<html:text property="WCMMetaTablecountryPaperApp/nextParticipantName"
		...
		%></td>
						...
						<html:hidden property="WCMMetaTablecountryPaperApp/nextParticipantId" />
					</table>
				</td>
			</tr>
			</td>
...
function returnNode(){
	...
	if(isSure==true){
		...
		try{
		document.getElementById('WCMMetaTablecountryPaperApp/nextParticipantName').value='<bean:write property="ExtendData/PassedNode/UserID"/>';
		}catch(e){
		
		}
		document.getElementById('WCMMetaTablecountryPaperApp/nextParticipantId').value='<bean:write property="ExtendData/PassedNode/UserID"/>';
	...
	var frm = document.forms[0];
		...
		if(validater(frm)) {
			if(getRadioValue("Flow/nextNode")!="EndActivity"){
				if(frm.elements["WCMMetaTablecountryPaperApp/nextParticipantId"].value == ""){
					alert("请选择下环节人员");
					return;
				}
			}
