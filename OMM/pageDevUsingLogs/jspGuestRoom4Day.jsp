	...
	<input  type="hidden" name="CurrDate" value="<bean:write property="input/CurrDate" formatType="yyyy-MM-dd" formatClass="com.primeton.eos.webtag.bean.BeanWriteDateFormatImpl"/>" />
		<table align="center">
			<tr>
  				...
  				<!--<bean:write property="input/BuildingName"/><bean:write property="input/LayerNO"/>-->
				<td nowrap class="td2">所属建筑物：<select name="input/BuildingID" id="select1" bizAction="ICCard.bizField.selectBuilding2"
						...
						mainSelectID="select1" nullLable="true" nullLableText="请选择楼宇"
...
function doMsg(obj){
	...
	function doClick(obj){
		...
		AddRoomID(obj.all["Room/RoomID"].value,obj.all["Room/RoomName"].value, document.all["BuildingName"].value);
	...
	function AddRoomID(roomID,RoomName,BuildingName){
        this.parent.window.opener.addOptionBtn2(trim(roomID),RoomName,BuildingName);
    }
	function doChange(){
        var frm = document.forms[0];
...
//------------------------------------------------------------------------------
	function changeBuilding(){
		...
		var CurrDate = frm.CurrDate.value;
		var op = "<bean:write property="op"/>";
//		window.location.href = "ICCard.prField.guestRoom4Day.do?input/BuildingID="+BuildingID+"&input/LayerNO="+LayerNO+"&op="+op;
//		frame.location.href = "ICCard.prField.guestRoom4Day.do?input/BuildingID="+BuildingID+"&input/LayerNO="+LayerNO+"&op="+op;
	}
