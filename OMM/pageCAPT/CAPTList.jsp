		...
		<tr>
			<td height="24" background="/internet/image/lz_bg.gif">
				<table width="100%" border="0" cellspacing="0" cellpadding="0">
					<tr>
						<td width="15"></td>
						<td class="text_wirte">运维管理—&gt;IT设备管理—&gt;设备库—&gt;导入设备</td>
					</tr>
				</table>
			</td>
		</tr>
					...
					<tr>
						<td valign="top" class="text">
							<TABLE border="0" width="98%" align="center">
								<TBODY>
									<TR>
										<TD height="500" width="180" align="left" style="padding-top:3px" valign="top">
											<tree:tree id="tree1" width="250" height="100%">
												<tree:root name="资产管理" icon="/internet/image/type1.gif" expandAction="oa.bizAssetsManage.typeList"
													firstLevelEntities="list[@type='CAPT_TYPE']" expand="true" onclick="queryRoot" ondblclick="reloadNodeChild" />
												<tree:display entity="CAPT_TYPE" expression="$[NAMECODE] " icon="/internet/image/type2.gif"
													onclick="queryGood" ondblclick="reloadNodeChild" />
												<tree:display entity="CAPT_CAPITALINFO" expression="$[CAPTNAME]" icon="/internet/image/type3.gif"
													onclick="viewGood" ondblclick="reloadNodeChild" />
												<tree:expand entity="CAPT_TYPE" action="oa.bizAssetsManage.infoList"
													childEntities="list[@type='CAPT_CAPITALINFO']" />
												<tree:move entity="CAPT_CAPITALINFO" toEntity="CAPT_TYPE" action="oa.bizAssetsManage.infoMove" />
												<tree:menu entity="CAPT_CAPITALINFO">
													<tree:menuitem name="添加到运维库" onclick="addItem" />
												</tree:menu>
											</tree:tree>
										</TD>
										<TD width="85%" align="left" valign="top">
											<FORM action="OMM.prCAPT.prImportCAPT.do" method="post" name="myForm"><input type="hidden" name="typeCode"
													value=""> <input type="hidden" name="modifytoSave" value="OMM.prCAPT.prImportCAPT.do">
													<input type="hidden" name="OMM_ITDevice/TypeID" value="-1">
												<table width="96%" border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#CBCBCB" id="type">
													<tr class="td_title">
														<td nowrap align="center">设备类型</td>
													</tr>
													<tr>
														<TD nowrap="nowrap" style="background-color:white;"><select name="typeID"
																onchange="getExtPro()">
												...
												<table width="96%" border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#CBCBCB"
													...
													<tr class="td_title">
														<td></td>
														<td width="30%" nowrap="nowrap" align="center">资产名称</td>
														<td width="8%" nowrap="nowrap" align="center">设备编号</td>
												...
												<TABLE>
													<TR align="left">
														...
														&nbsp;&nbsp;&nbsp;&nbsp;注意：1、输入格式：年份（两位）+流水号（三位），例：09001；
														</font></TD>
													</TR>
													<TR align="left">
														<TD class="td1"><font style='color:red;'>
...
var currentNode;
	...
	function addItem( treeNode ){
	 ...
	 // 增加明细表记录
	function insertDetail(treeNode) {
		...
		nameArray[i++] = "DevCode"; 
		nameArray[i++] = "OP";
		addRecordToTable(objTable, nameArray,treeNode);	
		rowArray[j++]=treeNode.entity.getProperty("CAPTID");	
	}
	
	var checkboxIndex = 0;  //递增索引号
	function addRecordToTable(objTable, nameArray,treeNode,index) {
		...
		for (var i=0; i<nameArray.length; i++) {
			...
			case 0: 
				colHTML += "<td>";
				colHTML += "<input type='hidden' name=\"list[@type='CAPT_CAPITALINFO']/CAPT_CAPITALINFO[@rowNum='"+rowArray.length+"']/CAPTID\" value='"+treeNode.entity.getProperty("CAPTID")+"' />";
				break;
			case 2: 
				colHTML += "<td>";
				colHTML += "<input type='text' name=\"list[@type='CAPT_CAPITALINFO']/CAPT_CAPITALINFO[@rowNum='"+rowArray.length+"']/code\" onkeypress=\"javascript:KeyPress(this);\" onblur='setDevCode(this);' />";
				break;
		 	case 3: //操作
				colHTML += "<td  nowrap align=\"center\">";
				colHTML += "<img  onClick=\"removeRowFromTable(document.all['listdetail'], 'row_"+checkboxIndex+"','"+treeNode.entity.getProperty("CAPTID")+"')\" src=\"/internet/image/delete1.gif\" style=\"cursor: hand\"> ";
				break;	
			default:
				colHTML += "<td  nowrap align=\"center\">";
				colHTML += treeNode.entity.getProperty("CAPTNAME");
				break;
			}
	...
	function removeRowFromTable(objTable, rowId,CAPTID) {
		var cells;
	...
	function save(){
		...
		if(captrows<2){alert("请选择需要导入的资产!");return false;}
