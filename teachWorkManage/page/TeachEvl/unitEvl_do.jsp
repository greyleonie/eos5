  ...
  for (int i=0;i<items.size();i++) {
     ...
     for (int j=0;j<levels.size();j++) {
        ...
        if (request.getParameter(objname) != null) {
              ...
              if (marks != null && marks.size() > 0) {
                 ...
                 sql = "Insert Into AssignMark (AssignID,LevelID,EvaluationID,ItemID,ClassID,CourseID,StudentID) Values("+maxid+","+levId+","+evlId+","+itemId+","+classID+","+unitID+","+studentID+")";
                 Statement stmt = conn.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
                 System.out.println(sql);
                 try {
                    stmt.executeUpdate(sql);
     ...
     if (request.getParameter(suggestobj) != null) {
        ...
        if (advices != null && advices.size() > 0) {
           ...
           List ids = dbsession.createSQLQuery(sql).list();
           int maxid = ((Integer)ids.get(0)).intValue();
           sql = "Insert Into Advice (AdviceID,EvaluationID,ClassID,itemID,CourseID,TargetType,Advice,StudentID) Values("+maxid+","+evlId+","+classID+","+itemId+","+unitID+",2,'"+suggest+"',"+studentID+")";
           Statement stmt = conn.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
           System.out.println(sql);
                 try {
                    stmt.executeUpdate(sql);
