...
<%
 ...
 try{
  ...
  String beg="";
  String end="";
  Object obj=request.getAttribute("countWeek");
 
 if(obj!=null) weekIndex=((Integer)request.getAttribute("countWeek")).intValue();
 ...
 %>
   ...
   if(week!=null&&week.equals(strWeekIndex)&&print){//是本周的才输出
       ...
       if(rowSpan!=null&&!rowSpan.equals("0")){//每日只输出一次
           %>
            <td rowspan='<%=rowSpan %>'><br><%=month %><br>月<br><%=day %><br>日</td>
...
function queryPlan(){
     ...
     if(beg==""&&end==""){
       frm.elements["exp"].value="0";
       frm.action="teachWorkManage.prMakeTeachPlan.allTeachPlan.do?sqlCondition&exp=0&sesson=1"+
       "&beg=&end=&conditionSql=&classIds=&isFirst=0&path=1";
       frm.submit();
       }
}
</script>