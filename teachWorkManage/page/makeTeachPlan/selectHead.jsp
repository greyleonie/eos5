...
function User(identify,id,name,dept,job,firstChar,workType,orgType,sex,isLeave,isDead,isRetire){
	...
	function compare_by_type(type,index){
		switch(type) {
			...
			case 'isLeave':
				return pattern.test(seperator + this.isLeave + seperator);
			case 'isDead':
				return pattern.test(seperator + this.isDead + seperator);
			case 'isRetire':
				return pattern.test(seperator + this.isRetire + seperator);
			case 'orgType':
  ...
  <table width="96%"  border="0" align="center" cellpadding="0" cellspacing="0" class="table">
	  <tr>
		<td><table width="96%"  border="0" align="center" cellpadding="0" cellspacing="0">
		  ...
		  <tr>
				<td height="30" align="center" class="text">单个人员选择
				...
				<table width="98%"  border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#A2C4DC" id="listdetail">
                  ...
                  <td height="25" align="center"> 部门： <html:select property="bmSelectList" size="1" style="width:200;" onchange="initSelect('dept')"> 
                    ...
                    <input name="sex" type="radio" value="1" onClick="initSex(this)">男&nbsp;<input name="sex" type="radio" value="0" onClick="initSex(this)">女
                                  <BR>
                                  已调离：<input type="checkbox" name="isLeave" onClick="initIsLeave(this)"/>
			  已退休：<input type="checkbox" name="IsRetire" onClick="initIsRetire(this)"/>
			  已故：<input type="checkbox" name="isDead" onClick="initIsDead(this)"/>
						</td>
					</tr>
					<tr class="td1" > 
					...
					<tr class="td1" >
                      ...
                      <tr align=center > 
                        <td> <fieldset>
                          <legend align="center" class="text_red">可选人员</legend>
                        ...
                        <td> <fieldset>
                          <legend align="center" class="text">已选人员</legend>
                          <div id="selContainer" > 
...
function initSelect(type) {
    ...
    case 'dept':
        	...
        	// alert("type:"+type+" orgValue:"+document.all.bmSelectList.value);
				for(user in userArray) {
	...
	case 'isRetire':
		setPattern(patternStr);
		for(user in userArray) {
				if(userArray[user].isMatch('isRetire') && userArray[user].isRight==false) {
					var option = new Option(userArray[user].name,userArray[user].id);
	...
	case 'isLeave':
		setPattern(patternStr);
		for(user in userArray) {
				if(userArray[user].isMatch('isLeave') && userArray[user].isRight==false) {
					var option = new Option(userArray[user].name,userArray[user].id);
	...
	case 'isDead':
		setPattern(patternStr);
		for(user in userArray) {
				if(userArray[user].isMatch('isDead') && userArray[user].isRight==false) {
					var option = new Option(userArray[user].name,userArray[user].id);
...
function initIsLeave(obj) {
	document.all.bmSelectList.value = "99999"
	document.all.orgTypeTemp.value = "-1"
	document.getElementById('isDead').checked=false;
	document.getElementById('isRetire').checked=false;
	var value=obj.checked?1:0;
	
	patternStr = seperator + value + seperator;
	initSelect('isLeave');
}

function initIsDead(obj) {
	document.all.bmSelectList.value = "99999"
	document.all.orgTypeTemp.value = "-1"
...
function initOrg(){
   var orgId='<bean:write property="orgId"/>'
   var org=document.forms[0].elements("bmSelectList");
   var count=org.length;
   
   for(var i=0;i<count;i++){
      var op=org.options[i].value;
      if(orgId==op){
        removeAll();
        org.selectedIndex=i;
...
function removeAll(){
   var man=document.forms[0].elements("leftSelect");
