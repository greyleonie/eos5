...
<body>
<form method="post" action="oa.prThingsManage.purchaseAddDo.do">
 ...
 <table width="100%"  border="0" cellspacing="0" cellpadding="0">
    <tr> 
	  ...
	  <table width="100%"  border="0" cellspacing="0" cellpadding="0" >
          <tr> 
            <td width="15"> </td>
            <td class="text_wirte">办公系统—&gt;用品管理—&gt;<span id="printTable">采购申请新增</span></td>
          </tr>
        </table></td>
    </tr>
    <tr>
      <td height="5">
		...
		<html:hidden  property="COMM_PURCHASE/FLAG" value="0" />
		<!--传递给选择用品层的参数-->
...
<!------------------------------  表单意见  ----------------------------->
           <table width="96%"  border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#A2C4DC">
                <tr> 
                  ...
                  <td width="10%" class="td1">&nbsp;<bean:write property="COMM_PURCHASE/PURCHASECODE"/> 
                    <html:hidden property="COMM_PURCHASE/PURCHASECODE"/> </td>
                  <td width="10%" align="right" class="td2">申请日期：</td>
                  <td width="10%" class="td1">&nbsp;<%=curdate%> <input type="hidden" name="COMM_PURCHASE/PURCHASEDATE[@pattern='yyyy-MM-dd']" value="<%=curdate%>"></td>
                   <td width="10%" align="right" class="td2">申请部门：</td>
                  <td width="10%" class="td1" nowrap="nowrap">&nbsp; <bean:write property="SessionEntity/orgName"/>
                  <html:hidden name="COMM_PURCHASE/BRANCH" property="SessionEntity/orgID"/> </td>
                  <td width="10%" align="right" class="td2">申 请 人：</td>
                  <td width="*" class="td1">&nbsp;<bean:write property="SessionEntity/userID" formatClass="username" formatType="userid"/> 
                    <html:hidden name="COMM_PURCHASE/PROPOSER"  property="SessionEntity/operatorID" /> 
                  </td>
              </table>
                <%
		             int count=0;
		  		...
		  		%><!--
              			<table width="96%"   border="0" align="center" cellpadding="0" cellspacing="1" id="buttonspace">
				            <tr>
			<td align="center" class="td1">领用申请库存不足用品（已审批）</td>       
              ...
              <table width="96%"  border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#CBCBCB" id="olddetail">
                ...
                <logic:iterate id="resultset" property="list[@type='COMM_SHORTAGEDETAILVIEW']"> 

				
               <tr class="td1" onClick="changeTRBgColor(this)" id="row_<%=count %>">  
                <td nowrap><bean:write id="resultset" property='COMM_SHORTAGEDETAILVIEW/COMMNAME'/>
                <input type="hidden" name="listInsert/COMM_PURCHASEDETAIL[@hciTagIndex='<%=count%>']/COMMNAME" value="<bean:write id="resultset" property='COMM_SHORTAGEDETAILVIEW/COMMNAME'/>" />
                <input type="hidden" name="listInsert/COMM_PURCHASEDETAIL[@hciTagIndex='<%=count%>']/DETAILID"  value="<bean:write id="resultset" property='COMM_SHORTAGEDETAILVIEW/DETAILID'/>"/></td> 
                <td nowrap><bean:write id="resultset" property='COMM_SHORTAGEDETAILVIEW/STOREOUTCODE'/>
                <input type="hidden" name="listInsert/COMM_PURCHASEDETAIL[@hciTagIndex='<%=count%>']/STOREOUTCODE" value="<bean:write id="resultset" property='COMM_SHORTAGEDETAILVIEW/STOREOUTCODE'/>" />
                <td nowrap><bean:write id="resultset" property='COMM_SHORTAGEDETAILVIEW/BRANCHNAME'/>
                <input type="hidden" name="listInsert/COMM_PURCHASEDETAIL[@hciTagIndex='<%=count%>']/BRANCHNAME"  value="<bean:write id="resultset" property='COMM_SHORTAGEDETAILVIEW/BRANCHNAME'/>" /></td>
                <td nowrap><bean:write id="resultset" property='COMM_SHORTAGEDETAILVIEW/PROPOSERNAME' />
                <input type="hidden" name="listInsert/COMM_PURCHASEDETAIL[@hciTagIndex='<%=count%>']/PROPOSERNAME"  value="<bean:write id="resultset" property='COMM_SHORTAGEDETAILVIEW/PROPOSERNAME' />"/></td>
                <td nowrap><bean:write id="resultset" property='COMM_SHORTAGEDETAILVIEW/SPECIA'/>
                <input type="hidden" name="listInsert/COMM_PURCHASEDETAIL[@hciTagIndex='<%=count%>']/SPECIA"  value="<bean:write id="resultset" property='COMM_SHORTAGEDETAILVIEW/SPECIA'/>"  /></td>
                <td nowrap><bean:write id="resultset" property='COMM_SHORTAGEDETAILVIEW/PRICE'/> 
                <input type="hidden" name="listInsert/COMM_PURCHASEDETAIL[@hciTagIndex='<%=count%>']/PRICE"  value="<bean:write id="resultset" property='COMM_SHORTAGEDETAILVIEW/PRICE'/>"/></td>
                <td nowrap><bean:write id="resultset" property='COMM_SHORTAGEDETAILVIEW/QUANTITY'/> 
                <input type="hidden" name="listInsert/COMM_PURCHASEDETAIL[@hciTagIndex='<%=count%>']/QUANTITY"  value="<bean:write id="resultset" property='COMM_SHORTAGEDETAILVIEW/QUANTITY'/>"/></td>
                <td nowrap><bean:write id="resultset" property='COMM_SHORTAGEDETAILVIEW/AMOUNT'/> 
                <input type="hidden" name="listInsert/COMM_PURCHASEDETAIL[@hciTagIndex='<%=count%>']/AMOUNT"  value="<bean:write id="resultset" property='COMM_SHORTAGEDETAILVIEW/AMOUNT'/>"/></td>
                <td nowrap align="center"><img  onClick='removeRowFromTable(document.all["olddetail"], "row_<%=count %>")' src="/internet/image/delete1.gif" style="cursor: hand"></td>
              </tr>
				</logic:iterate>
              </table>	
          --><table width="96%"   border="0" align="center" cellpadding="0" cellspacing="1" >
				            <tr>
			<td align="center" class="td1">常规采购用品</td>       
...
function send() {
    ...
    //	var oldTable = document.all['olddetail'];  
    	var rowIndexId = 0;
    	var rowObj;
...
// 增加明细表记录
	...
	function addNewRecordToTable(objTable, nameArray) {
		...
		for (var i=0; i<nameArray.length; i++) {
			...
			case  0: // 用品名称
				colHTML += "<td  nowrap>";
				colHTML += "<input type=\"text\" name=\"" + listName + checkboxIndex + "'" + "]/COMMNAME\"  onclick=\"selectGood(this,"+checkboxIndex+")\"  onkeyup=\"selectGood(this,"+checkboxIndex+")\" onblur=\"f_OnBlur()\"  style=\"width:100%\" class=\"inputOA\" readOnly />";
