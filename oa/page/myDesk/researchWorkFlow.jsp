...
tools.util.DeskInfo.queryDeskInfo(request,userId,username,"researchWorkFlow");
...
ArrayList dbWorkFlowList = (ArrayList)request.getAttribute("researchWorkFlowList");
String dbWorkFlowRecords = "0";
dbWorkFlowRecords=(String)request.getAttribute("researchWorkFlowRecords");
                ...
                %>
                 <%i++; %>
                	...
                	<table width="95%"  border="0" cellspacing="0" cellpadding="0">
	                    <tr>
	                      <td height="24" bgcolor="white" ><img src="/internet/images/index_meun02.gif" alt="" width="3" height="3" hspace="8" align="absmiddle"><a href="javascript:toFlowLink('<%=dbWorkFlowData.getTypeFlag() %>',<%=dbWorkFlowData.getChannelID() %>,<%=dbWorkFlowData.getFlowID() %>,'<%=dbWorkFlowData.getType() %>','db')"><font tips='<%=content %>' onmousemove="show(this.tips,1);" onmouseout="show(this.tips,0)"><%=title %></font>
...
<div id="mytips" name="mytips" style="line-height:20px;font-weight:bold;position:absolute;width:150;height:16;border:1 gray solid;font-size:12px;background-color:#ffffff;color:red;display:none;filter: progid:DXImageTransform.Microsoft.Shadow(color=#999999,direction=135,strength=3);"></div>
</body>
...
var tmp='<bean:write property="trs" />';
	...
	if(ctype==2){
		...
		var index=url.indexOf("/wcm/");
		url=url.substring(0,index);
		//window.open('http://wcm65.gzswdx.gov.cn:9090/wcm/dxapps/paperapply/paperapply_' + subaction);
		var tmp='/wcm/dxapps/paperapply/paperapply_';
		if(flow=='wfFunding')
			tmp='/wcm/dxapps/expensemanage/expensemanage_';
		if(flow=='wfProject')
			tmp='/wcm/dxapps/probuildverify/probuildverify_';
		toopenwin(url + tmp + subaction);	
	}else{
	if(tmp==1){
		//var h="common.pr.baseTopFrame.do?menuID="+leftID+"&subid="+subID+"&subaction="+subaction+"&flag="+flag;
		//var h= "common.pr.baseTopFrame.do?menuID="+leftID+"&subid="+subID+"&subaction="+subaction+"&flag="+flag;
