...
<body>
<form method="post" name="myform" action="oa.prMeetingManage.mrApplyProcessDo.do" enctype="multipart/form-data">
<table width="100%"  border="0" cellspacing="0" cellpadding="0">
    <tr> 
	  ...
	  <table width="100%"  border="0" cellspacing="0" cellpadding="0">
          <tr> 
            <td width="15"> </td>
            <td class="text_wirte">办公系统—&gt;会议管理—&gt;会议申请—&gt;会议申请处理流程—&gt;<bean:write property="WFWorkItem/workItemName"/></td>
          </tr>
        </table></td>
    </tr>
    <tr>
      <td height="5">
		...
		<html:hidden property="MR_APPLY/APPLYID" />
		<html:hidden property="MR_APPLY/participant" />
		<html:hidden property="MR_APPLY/passParticipant" />
		<html:hidden property="MR_APPLY/ConferencePOTID" />
		<html:hidden property="MR_APPLY/Organizer" />
	...
	<tr>
		<td height="100%" valign="top" nowrap bgcolor="#FFFFFF" style="border:1px #000000 solid;border-top:none">
			...
			<table width="96%"  border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#A2C4DC" id="basedata">
			  <tr> 
				...
				<td width="40%" nowrap class="td1"> <html:text property="MR_APPLY/APPLYDATE[@pattern='yyyy-MM-dd']"   attributesText='class="input" readonly="true"' onclick="calendar(this,'yyyy-MM-dd')" size="20" />				 			</td>
			  </tr>
			  
			  <tr> 
				<td height="25" align="center" nowrap class="td1">会议名称：</td>
				<td colspan="3" nowrap class="td1"><html:text property="MR_APPLY/MeetingName"   attributesText='class="input"  ValidateType="notempty" Msg="会议名称不能为空"' size="80" maxlength="100"/></td>
			  </tr>
			  <tr>
			    <td height="25" align="center" nowrap class="td1">会议主题：</td>
			    <td colspan="3" nowrap class="td1"><html:text property="MR_APPLY/TOPIC"   attributesText='class="input"' size="80" maxlength="100"/></td>
		      </tr>
			  <tr>
			  ...
			  <tr>
			    ...
			    <td colspan="3" nowrap class="td1"><html:text property="MR_APPLY/Moderator"   attributesText='class="input"' size="80" maxlength="100"/>
			  ...
			  <tr>
			    ...
			    <td colspan="3" nowrap class="td1"><html:text property="MR_APPLY/MRParticipants"   attributesText='class="input"' size="80" maxlength="100"/></td>
		      </tr>
			  <tr>
			    <td height="25" align="center" nowrap class="td1">开始时间：</td>
			    <td nowrap class="td1"><html:text property="MR_APPLY/STARTDATE[@pattern='yyyy-MM-dd HH:mm:ss']"   attributesText='class="input" readonly="true"' onclick="calendar(this,'yyyy-MM-dd HH:mm:ss')" size="20" /></td>
		        <td align="center" nowrap class="td1">结束时间：</td>
		        <td nowrap class="td1"><html:text property="MR_APPLY/ENDDATE[@pattern='yyyy-MM-dd HH:mm:ss']"   attributesText='class="input" readonly="true"' onclick="calendar(this,'yyyy-MM-dd HH:mm:ss')" size="20" /></td>
	          </tr>
			  <tr>
			    <td height="25" align="center" nowrap class="td1">会议室：</td>
			    <td colspan="3" nowrap class="td1">
			    <html:hidden property="MR_APPLY/MRCODE"/>
			    <html:text property="MR_APPLY/MRNAME"   attributesText='class="input" readonly="true" ValidateType="notempty" Msg="会议地点不能为空"' size="80" maxlength="100"/>
			  ...
			  <tr>
			    ...
			    <td colspan="3" nowrap class="td1"><fcc:wfdata name="niBanOption" wfnode="DX2" type="textarea" list="list[@type='wfdata']" domwfnode="MR_APPLY/activityDefID" cols="80" rows="5" classname="inputOA" onmousedown="menuControl('pubLemma',1)" onmouseout="menuControl('pubLemma',0)"/></td>
		      </tr>
			  <tr>
			    <td height="25" align="center" nowrap class="td1">办公室办理意见：</td>
			    <td colspan="3" nowrap class="td1"><fcc:wfdata name="niBanOption" wfnode="DX3" type="textarea" list="list[@type='wfdata']" domwfnode="MR_APPLY/activityDefID" cols="80" rows="5" classname="inputOA" onmousedown="menuControl('pubLemma',1)" onmouseout="menuControl('pubLemma',0)"/></td>
		      </tr>
			</table>
      ...
      <table width="96%"  border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#A2C4DC" id="participant">
		...
		<tr> 
		  ...
		  <input name="select" type="button" class="button_02" value="选 择" onClick="selectParticipant('<bean:write property="WFWorkItem/processInstID"/>','<bean:write property="WFWorkItem/processDefID"/>','10000','MR_APPLY/nextParticipantName','MR_APPLY/nextParticipantId')">
		  <%}else{ %>
		  <html:text property="MR_APPLY/nextParticipantName" attributesText='style="display:none;width:80%;border:1 #efefef soild;" readonly="true"' />
		  
		  <input name="select" type="button" class="button_02" value="选 择" style="display:none;" onClick="selectParticipant('<bean:write property="WFWorkItem/processInstID"/>','<bean:write property="WFWorkItem/processDefID"/>','10000','MR_APPLY/nextParticipantName','MR_APPLY/nextParticipantId')">
		  <%} %>
		  
		  </td>
        </tr>
        	  </logic:notEqual><html:hidden property="MR_APPLY/nextParticipantId"/>
      </table>
...
function returnNode(){
	...
	if(isSure==true){
		...
		try{
		document.getElementById('MR_APPLY/nextParticipantName').value='<bean:write property="ExtendData/PassedNode/UserID"/>';
		}catch(e){
		
		}
		document.getElementById('MR_APPLY/nextParticipantId').value='<bean:write property="ExtendData/PassedNode/UserID"/>';
...
function  data(){
     var frm = document.forms[0];
     var startdate=frm.elements["MR_APPLY/STARTDATE[@pattern='yyyy-MM-dd HH:mm:ss']"].value;
   
     var enddate=frm.elements["MR_APPLY/ENDDATE[@pattern='yyyy-MM-dd HH:mm:ss']"].value;
