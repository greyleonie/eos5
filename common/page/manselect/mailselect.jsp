...
String userID = reqContext.getEntityValue("SessionEntity/operatorID");
%>
...
function User(identify,id,name,dept,job,firstChar,workType,orgType,sex,email){
...
//标识，所属用户，组别，姓名，电邮，手机号码，首字母，性别
function AddressList(CARDID,OwnerID,GroupID,CUSTOMER,EMAIL,MOBILE,FirstLetter,Sex){
	this.id = CARDID;
	this.owner = OwnerID;
	this.gid = GroupID;
	this.name = CUSTOMER;
	...
	function compare_by_type(type,index){
		switch(type) {
			...
			case 'sex':
				return pattern.test(seperator + this.sex + seperator);
			case 'owner':
			    return pattern.test(seperator + this.owner + seperator);
			default:
				return pattern.test(this.name);
		}
	}
...
//标识，所属用户，组别，姓名，电邮，手机号码，首字母，性别
	var addressArray = new Array()
	 <logic:iterate id="address" property="list[@type='AddressList']">
	   addressArray[addressArray.length] = new AddressList('<bean:write id="address" property="AddressList/CARDID" />','<bean:write id="address" property="AddressList/OwnerID" />','<bean:write id="address" property="AddressList/GroupID" />','<bean:write id="address" property="AddressList/CUSTOMER" />','<bean:write id="address" property="AddressList/E_MAIL" />','<bean:write id="address" property="AddressList/MOBILE" />','<bean:write id="address" property="AddressList/FirstLetter" />','<bean:write id="address" property="AddressList/Sex" />');
	 </logic:iterate>
  ...
  <table width="96%"  border="0" align="center" cellpadding="0" cellspacing="0" class="table">
	  <tr>
		<td><table width="96%"  border="0" align="center" cellpadding="0" cellspacing="0">
		  <tr>
				<td height="30" align="center" class="text">多个人员选择</td>
		  </tr>
			  ...
			  <td valign="top" class="text">
				<table width="98%"  border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#A2C4DC" id="listdetail">
                  ...
                  <td height="25" align="left"> &nbsp;&nbsp;&nbsp;
                    部门: <html:select property="bmSelectList" size="1" style="width:200;" onchange="initSelect('dept')"> 
                    <script>
					...
					<tr class="td2" > 
					  <td height="25" align="left">&nbsp;&nbsp;&nbsp;
					    通讯录:
					  	<input name="addresstype" type="radio" value="-1" onClick="initAddress('all','')">全部&nbsp;<input name="addresstype" type="radio" value="1" onClick="initAddress('public','0')">公共&nbsp;<input name="addresstype" type="radio" value="2" onClick="initAddress('private','<%=userID%>')">本人
						</td>
					</tr>
					<tr class="td1" > 
...
function initSelect(type) {
        	...
        	if(document.all.bmSelectList.value != 99999) {
				...
				for(user in userArray) {
						if(userArray[user].isMatch('dept') && userArray[user].isRight==false) {
							...
							var option = new Option(userArray[user].name,userArray[user].email);
							from.add(option);
						}
				}
        	}else {
	...
	case 'letter':
		...
		if (listtype!='addresslist') {
		   ...
		   for (address in addressArray) {
		       if (addressArray[address].isMatch('letter') && addressArray[address].isRight==false) {
		          var option = new Option(addressArray[address].name,addressArray[address].email);
		          from.add(option);
		       }
		   }
		}
	...
	case 'sex':
		...
		for(user in userArray) {
		   ...
		   setPattern(patternStr);
		   for (address in addressArray) {
		       if (addressArray[address].isMatch('sex') && addressArray[address].isRight==false) {
		          var option = new Option(addressArray[address].name,addressArray[address].email);
		          from.add(option);
		       }
		   }
	...
	case 'all':
		setPattern(patternStr);
		for (address in addressArray) {
		    if (addressArray[address].isRight==false) {
		       var option = new Option(addressArray[address].name,addressArray[address].email);
		       from.add(option);
		    }
		}
	...
	case 'public':
		setPattern(patternStr);
		for(address in addressArray) {
		   if (addressArray[address].isMatch('owner') && addressArray[address].isRight==false) {
		      var option = new Option(addressArray[address].name,addressArray[address].email);
		      from.add(option);
		   }
		}
	...
	default:
		...
		/**for(address in addressArray) {
		   var option = new Option(addressArray[address].name,addressArray[address].email);
		   if (addressArray[address].isRight) {
		      selArr[addressArray[address].index].add(option);
		   } else {
		      from.add(option);
		   }
		}*/
...
function initAddress(type,owner) {
   listtype = 'addresslist';
   document.all.bmSelectList.value = "99999"
   document.all.orgTypeTemp.value = "-1"
   
   patternStr = seperator + owner + seperator;
   initSelect(type);
}
...
function moveOneFrom(type) {
	...
	for(var i = 0 ; i < from.options.length ; i ++) {
		if(from.options[i].selected) {
		  if ((from.options[i].value != '' && isEmail(from.options[i].value)) || type=='right') { 
		    ...
		    alert(from.options[i].text + "的个人信息中没有设置邮箱地址或地址有误，可退出后直接输入，用','隔开！");
		  }
		}
	}
}
...
function setRight(type) {
   ...
   for(address in addressArray) {
      addressArray[address].isMatch('id_' + type);
   }
}
	...
	if (listtype!='addresslist') {
	  for(user in userArray) {
		...
		if(userArray[user].isMatch() && userArray[user].isRight == false) {
			var option = new Option(userArray[user].name,userArray[user].mail);
	  ...
	  for(address in addressArray) {
		...
		if(addressArray[address].isMatch() && addressArray[address].isRight == false) {
			var option = new Option(addressArray[address].name,addressArray[address].mail);
...
for(var i=0;i<userIds.length;i++){
    ...
    for(address in addressArray) {
		if(!addressArray[address].isRight){
			for(var j = 0 ; j < idArr.length ; j++) {
				setPattern(";" + idArr[j] + ";");
				addressArray[address].isMatch('id_left',i);
			}
		}
	}
}
...
initSelect();
<logic:present property="orgID">
	document.all.bmSelectList.value = <bean:write property="orgID"/>;
	initSelect('dept');
</logic:present>
</script>