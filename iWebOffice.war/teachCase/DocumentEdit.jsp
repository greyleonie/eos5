...
<%
  ...
  System.out.println("====mEditType:"+mEditType);
  String mShowType=request.getParameter("ShowType");
  String purview=request.getParameter("Purview");
  String mUserName= "";
  if (request.getParameter("UserName") != null)
     mUserName=new String(request.getParameter("UserName").getBytes("GBK"));
  ...
  if (DbaObj.OpenConnection())
  {
	...
	//初始化值
        ...
        mTemplate=mTemplate;
		if (request.getParameter("SendArchive/archiveTitle")!=null)
           mSubject=new String(request.getParameter("SendArchive/archiveTitle").getBytes("8859_1"));
        else
           mSubject="test";
		mAuthor=mUserName;
...
function Load(){
  ...
  //参数一:Index按钮编号,参数二:Caption按钮显示内容,参数三:Icon图标名称(标识最多到28)
//  webform.WebOffice.AppendTools("5","签名印章",13); //在OnToolsClick中的 vIndex=11 ,vCaption="工具栏11";
//  webform.WebOffice.AppendTools("6","正文套红",21);
  //webform.WebOffice.AppendTools("9","工具栏12",9); //在OnToolsClick中的 vIndex=12 ,vCaption="工具栏12";
  //webform.WebOffice.AppendTools("10","-",0);
  <%
	if ("postil".equals(purview)) {
  %>
  webform.WebOffice.VisibleTools("新建文件,打开文件,保存文件,文档清稿",false);
...
<body bgcolor="#ffffff" onload="Load()" onunload="UnLoad()">  <!--引导和退出iWebOffice-->
  <form name="webform" method="post" action="DocumentSave.jsp" onsubmit="return SaveDocument();">  <!--保存iWebOffice后提交表单信息-->
    ...
    <input type="hidden" name="ShowType" value="<%=mShowType%>">
    <input type="hidden" name="HTMLPath" value="<%=mHTMLPath%>">
    <input type="hidden" name="Readonly" value="false">
	<input type="hidden" name="FileDate" value="<%=FormatDate(mFileDate,"yyyy-MM-dd")%>">
    <table border=0  cellspacing='0' cellpadding='0' width=100% height=100% align=center class="TBStyle">
      <tr>
        ...
        <td colspan="2" class="TDStyle" height=90%>
           <table border=0 cellspacing='0' cellpadding='0' width='100%' height='100%' >
             <tr>
                <td bgcolor=menu>
                  <!--调用iWebOffice，注意版本号，可用于升级-->
                  <div id="DivID">
				  <object id="WebOffice" name="WebOffice" width="100%" height="100%" classid="clsid:8B23EA28-723C-402F-92C4-59BE0E063499" codebase="../iWebOffice2006.cab#version=7,5,0,0">
				  </object>
				  </div>
                </td>
             </tr>
...
if(frm.elements["TeachingCase/Theme"].value == "") {
 alert("主题不能为空！");
		return;
 }
if(frm.elements["TeachingCase/Title"].value == "") {
 alert("题目不能为空！");
		return;
 }
}catch(e){}
...
if (request.getParameter("STAT") != null) {
	if ("SAVEOK".equals(request.getParameter("STAT"))) {
	if (request.getParameter("RecordID") != null) {
       String mDocumentID=request.getParameter("RecordID");
%>
var parentForm=window.parent.document.forms[0];

parentForm.elements["TeachingCase/RecordID"].value='<%=mRecordID%>';
